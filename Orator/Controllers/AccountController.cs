        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
					var user = _dbContext.GetUserByEmail(model.Email);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
					...
					//create the profile user account
					UserResponse userResponse = _dbContext.CreateUser(new Models.Users.User
					{
						AuthId = user.Id,
						Email = user.UserName,//TODO for now email is also username
						Username = user.UserName,
						CreatedAt = System.DateTime.UtcNow,
						UpdatedAt = System.DateTime.UtcNow
					});

					CreateUserCookie(userResponse.Id, user.UserName, model.Email);
